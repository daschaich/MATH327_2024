...
\renewcommand{\moddate}{Last modified 23 Feb.~2024}
...
Extensive quantities are formally defined by considering how they behave if two isolated subsystems are \textit{analyzed} as a single system --- while still remaining isolated from each other, exchanging neither energy nor particles.
This is clearest to consider through the specific example shown below of two isolated spin subsystems, $\Om_1$ \& $\Om_2$, respectively characterized by the corresponding energies $E_1$ \& $E_2$ and particle numbers $N_1$ \& $N_2$.
...
\begin{mdframed}
  $S_{1+2} = $ \\[100 pt] % WARNING: ADJUSTED SIZE BY HAND TO FILL MORE OF THE PAGE
\end{mdframed}
...
\begin{itemize} % Adapted from a tutorial activity since tutorial replaced by computer lab...
