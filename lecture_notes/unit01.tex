...
\renewcommand{\moddate}{Last modified 9 Feb.~2024}
...
% This operates behind the scenes when we talk about $\{\om_1, \cdots, \om_N\}$ etc....
...
The scale of fluctuations (or the `uncertainty') around the expected final position of the walker also depends on time:
\begin{equation*}
...
\begin{shaded}
  ...
  \begin{equation}
    ...
    \De x = D \sqrt{t},
  \end{equation}
  where $D$ is the \textbf{diffusion constant} and $\De x$ is sometimes called the \textit{diffusion length}.
\end{shaded}
...
(As in \secref{sec:probdist}, we can extract this probability by integrating the distribution $p(x)$ over an appropriate interval.)
...
\end{center}
In the figure on the left, the large diffusion constant $D \approx 0.98$ causes each individual probability distribution to look similar to the corresponding one for $\vdr = 0$, but now their central peaks (and expectation values $\vev{x}$) drift steadily to the right.
The lower $D \approx 0.199$ for the figure on the right causes the distributions to look a bit different, but they still diffuse to exhibit lower and broader peaks as time passes.
